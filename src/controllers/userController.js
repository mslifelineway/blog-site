...
exports.getAllUsers = async (req, res, next) => {
  try {
    ...
    return res
      ...
      .json({ result: _copy(users), message: messages.allUsersFetched });
  } catch (e) {
    next(e);
  }
};
...
exports.softDeleteUserById = async (req, res, next) => {
  try {
    ...
    return res
      ...
      .json({ message: messages.userDeleted });
  } catch (e) {
    const { path } = e;
    if (path) {
      e.message = messages.invalidUserId;
  ...
  try {
    ...
    if (updatedUser) {
      return res.status(statusCodes.success).json({
        result: updatedUser,
        message: !status ? messages.userBlocked : messages.userUnblocked,
      });
    }
    return next({
      message: messages.userNotExists,
